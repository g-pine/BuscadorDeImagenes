...
function buscarImagenes(){
    ...
    const key = '36467585-7fc520774c842e653318c29e9';
    const url = `https://pixabay.com/api/?key=${key}&q=${termino}&per_page=${registroPorPagina}&page=${paginaActual}`;

    fetch(url)
        .then(respuesta => respuesta.json())
        .then(resultado => {
            totalPagina = calcularPaginas(resultado.totalHits);
            console.log(totalPagina);
            mostrarImagenes(resultado.hits);
        })
}

//generador resgistrara elementos por pagina
...
function mostrarImagenes(imagenes){
    ...
    imagenes.forEach (imagen => {
        ...
        resultado.innerHTML += `
        <div class="block w-1/2 md:w-1/3 lg:w-1/4 p-3 mb-4">
            <div class="bg-white">    
                <img class="w-full" src="${previewURL}">
                <div class="p-4">
                <p class="font-bold">${likes} <span class="font-light">Me Gusta</span></p>
                <p class="font-bold">${views} <span class="font-light">Veces vista</span></p>
                <a class="block w-full bg-blue-800 hover:bg-blue-500 text-whie uppercase font-bold text-center rounded mt-8 p-1" 
...
function imprimirPaginador(){
    ...
    while(true){
        ...
        //generando boton por cada elemento del generador
